...
parseTRAFile <- function(path){
  ...
  idx <- c(1,which(is.na(inh_data[[2]]),arr.ind = TRUE))
  ...
  idx <- c(1,which(is.na(oral_data[[2]]),arr.ind = TRUE))
  ...
  idx <- c(1,which(is.na(dermal_data[[2]]),arr.ind = TRUE))
...
parseConsExpoFile <- function(path){
  print(path)
  f <- readLines(file(description = path),warn = FALSE,skipNul = TRUE)
  f <- f[-which(f=="")]
